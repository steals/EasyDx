  ...
  render() {
    ...
    if (this.state.searchValue !== '') {
      ...
      for (let i = 0; i < this.props.orgs.length; i++) {
        const username = this.props.orgs[i].username.toLowerCase();
        let defaultMarker = '';
    ...
    const currentOrgs = filteredSource.slice(indexOfFirstItem, indexOfLastItem);

    let orgType = 'nonScratch';
    if (this.props.title === 'Scratch Orgs') {
      orgType = 'scratch';
    }
    const orgRows = currentOrgs.map(org => (
      <OrgRow
        key={org.orgId}
        org={org}
