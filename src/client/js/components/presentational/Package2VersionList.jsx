  ...
  render() {
    ...
    if (this.state.searchValue !== '') {
      ...
      for (let i = 0; i < this.props.versionList.length; i++) {
        const versionname = this.props.versionList[i].Name.toLowerCase();
        const versionNumber = this.props.versionList[i].Version;
    ...
    const maxCount = Math.ceil(filteredSource.length / numberPerPage);
    const indexOfLastItem = this.state.currentPage * numberPerPage;
    const indexOfFirstItem = indexOfLastItem - numberPerPage;
    const currentRecords = filteredSource.slice(indexOfFirstItem, indexOfLastItem);

    const package2VersionRows = currentRecords.map(package2Version => (
      <Package2VersionRow key={package2Version.Id} curVersion={package2Version} />
    ));
        ...
        <div className="card-body">
          ...
          <div className="table-responsive">
            <table className="table table-striped">
              ...
              <tbody>{package2VersionRows}</tbody>
            </table>
          </div>
          <Pagination currentPage={this.state.currentPage} pageClick={this.pageClick} maxCount={maxCount} />
          <button type="button" className="btn btn-primary" onClick={this.props.handleRefreshVersions}>
            Refresh Package Version List
          </button>
        </div>
      </div>
    );
