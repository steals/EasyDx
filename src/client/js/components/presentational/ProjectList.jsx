...
class ProjectList extends Component {
  ...
  render() {
    ...
    const maxCount = Math.ceil(this.props.projects.length / numberPerPage);
    const indexOfLastItem = this.state.currentPage * numberPerPage;
    const indexOfFirstItem = indexOfLastItem - numberPerPage;
    const currentProjects = this.props.projects.slice(indexOfFirstItem, indexOfLastItem);

    const projectRows = currentProjects.map(project => (
      <ProjectRow
