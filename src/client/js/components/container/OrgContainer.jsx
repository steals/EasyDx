      ...
      .then(res => {
        if (res.status === 200) {
          ...
          this.showAlertMessage(
            'success',
            'Please input your user name and password in the opening tab and you will be connected successfully!'
          );
        } else {
          this.toggleLoadingImage(false);
          this.showAlertMessage('danger', `Error:${res.data.err}`);
    ...
    axios
      ...
      .then(res => {
        if (res.status === 200) {
          ...
          for (let i = 0; i < this.state.scratchOrgs.length; i += 1) {
          ...
          for (let i = 0; i < this.state.nonScratchOrgs.length; i += 1) {
