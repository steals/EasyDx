      ...
      .then(res => {
        if (res.status === 200) {
          ...
          this.showAlertMessage(
            'success',
            'Changes pushed successfully into org! Please view details at the bottom of the page'
          );
        } else {
          this.toggleLoadingImage(false);
          this.showAlertMessage(
            'danger',
            `Error: ${res.data.message} Please see detailed results at the bottom of the page`
          );
          const { result, partialSuccess } = res.data;
        ...
        <div className="container-fluid">
          <div className="row">
            <div className="col-md-12 col-lg-8">
              ...
              <SourceListCard
                remoteChanges={this.state.remoteChanges}
                localChanges={this.state.localChanges}
                handleRefreshStatus={this.handleRefreshStatus.bind(this)}
              />
              {this.state.showFailedResult ? (
                <DeployFailedResult title="Deployment Failures" deploymentFailures={this.state.deploymentFailures} />
              ) : null}
              {this.state.showPushedSource ? (
                <ProjectConvertResult title={this.state.pushedTitle} convertResults={this.state.pushedSource} />
              ) : null}
            </div>
